class Solution {
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        // introduce pac and atl set to store the value that which cell can reach the ocean
        Set<List<Integer>> pac = new HashSet<>();
        Set<List<Integer>> alt = new HashSet<>();
        ...
        for(int r = 0; r < row; r++){
            for(int c = 0; c <col; c++){
                List<Integer> pCord = Arrays.asList(r,c);

                if(pac.contains(pCord) &&
                   alt.contains(pCord)){
                    ans.add(pCord);
                }
            }
    ...
    private void dfs(int r, int c, Set<List<Integer>> set, int prevHeight, int[][] heights){
